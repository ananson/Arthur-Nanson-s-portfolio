...
Classe qui sert a afficher tout a l'ecran
"""
# PyQt
...
class App(QMainWindow):
    ...
    def initUI(self):
        ...
        self.replay_button = QPushButton("Regarder un replay", self)#bouton qui sert a regarder une partie deja jouee

        self.label_video = QLabel()
        self.replay_button.clicked.connect(self.get_file)
        self.settings_grid.addWidget(self.replay_button, 3, 1)
        self.settings_grid.addWidget(self.label_video,4,0)


        self.record_button = QPushButton("Record Game")
        self.settings_grid.addWidget(self.record_button,3,0)

        label = QLabel('Player 1:')
        self.settings_grid.addWidget(label, 0, 0, alignment=Qt.AlignRight)
        self.cb_player1 = QComboBox()
        self.cb_player1.addItems(['Minimax', 'Human', 'Tournoi'])
        ...
        self.cb_player2.addItems(['Minimax', 'Human', 'Tournoi'])
    ...
    def get_file(self):
        ...
        """Permet de choisir un fichier a lire
        """
        fichier, _ = QFileDialog.getOpenFileName(self, 'Open video file', '\home')
        self.label_video.setPixmap(QPixmap(fichier))
    ...
    def make_player(text, player_id, board):
        ...
        if text == 'Minimax':
            ...
            return TournoiAIPlayer(player_id, board)
        else:
    ...
    def toggle_players(self):
        ...
        if isinstance(self.current_player, AIPlayer):#Si un IA joue, demarre le timer pour ne pas depasser une seconde
            debut = time.time()
        ...
        if isinstance(self.current_player, AIPlayer):
            ...
            if fin - debut > 1:
                self.exhibit_end_of_game(False)
                return

    def exhibit_end_of_game(self, won=True):
        '''
        ...
        else:#game pas finie mais IA a prit trop de temps
            popup = WinnerPopup([1,2,3], self)
            popup.exec_()
