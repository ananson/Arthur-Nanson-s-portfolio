...
Code qui est le coeur du jeu, verifie si la partie est finie et fais jouer les joueurs
"""
from utils import *
import copy
class Action:
...
class Board:
    ...
    def reset(self):
        ...
        self.played_boards = [] #tous les plateaux deja joues
        self.played_offsets = []
    ...
    def get_valid_actions(self, player):
        ...
        for i in actions: #enleve les plateaux deja joues
            self.new_grid = copy.deepcopy(self.grid)# variables temporaires qui servent a comparer avec les plateaux deja joues
            self.new_offset = copy.deepcopy(self.offset)
            if i.is_placement():
                self.new_grid[i.row][i.col] = player
            else:
                self.new_offset[i.row] += i.direction

            if (self.new_grid, self.new_offset) in self.played_boards:#offset et grid deja joues donc on enleve ce mouvement
                if i.is_placement:
                    actions.remove(i)

                else:
                    actions.remove(Action(i.row, col=None, direction = -i.direction))
