...
@section('content')
    <div class="container">
        <h2>Modifier la dépense "{{ $expense->title }}"</h2>
        @if (session('error'))
            <div class="alert alert-danger">
                {{ session('error') }}
            </div>
        @endif
        <form method="POST" action="{{ route('updateExpense', ['expense_id' => $expense->expense_id]) }}">
            @csrf
            @method('PUT')
            ...
            <button type="submit" class="btn btn-primary">Mettre à jour la dépense</button>
        </form>
    </div>
@endsection
